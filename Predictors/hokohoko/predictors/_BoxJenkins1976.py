#   hokohoko/predictors/_BoxJenkins1976.py
#
...
class BoxJenkins1976(Predictor):
    """
    This Predictor is based on the ARIMA algorithm expounded by Box & Jenkins in 1976.

    See Durbin 1960.

    It is a wrapper for ARIMA from statsmodels.
    """
    ...
    def __init__(
        ...
        # # 0. Arguments
        # ap = ArgumentParser()
        # ap.add_argument('--window', type=int, default=BoxJenkins1976.DEFAULT_WINDOW)
        # ap.add_argument('--order', type=tuple, default=BoxJenkins1976.DEFAULT_ORDER)
    ...
    def on_bar(self, bars: Iterable[Bar]) -> None:
        ...
        #         up_fit = ARIMA(np.array(self._highs[b.symbol_id]), self._order).fit(disp=0, trend='nc')
        #         down_fit = ARIMA(np.array(self._lows[b.symbol_id]), self._order).fit(disp=0, trend='nc')
