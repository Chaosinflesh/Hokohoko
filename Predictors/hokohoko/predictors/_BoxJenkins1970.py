#   hokohoko/predictors/_BoxJenkins1970.py
#
...
#   ====================================================================
#
#   This is a predictor based on an implementation of Box and Jenkins'
#   1970 ARMA model.
...
class BoxJenkins1970(Predictor):
    """
    This Predictor is based on the ARMA algorithm expounded by Box & Jenkins in 1970.

    It is a wrapper for ARMA from statsmodels.
    """
    ...
    def __init__(
        ...
        # 0. Arguments
        # ap = ArgumentParser()
        # ap.add_argument('--window', type=int, default=BoxJenkins1970.DEFAULT_WINDOW)
        # ap.add_argument('--order', type=tuple, default=BoxJenkins1970.DEFAULT_ORDER)
        ...
        #
        # # 2. ARMA configuration
        # self._window = self.parameters.window
        # self._order = self.parameters.order
        # self._highs = {}
        # self._lows = {}
    ...
    def on_bar(self, bars: Iterable[Bar]) -> None:
        ...
        #         up_fit = ARMA(np.array(self._highs[b.symbol_id]), self._order).fit(disp=0, trend='nc', transparams=False, tol=1e-6)
        #         down_fit = ARMA(np.array(self._lows[b.symbol_id]), self._order).fit(disp=0, trend='nc', transparams=False, tol=1e-6)
